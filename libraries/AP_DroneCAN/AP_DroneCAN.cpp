...
const AP_Param::GroupInfo AP_DroneCAN::var_info[] = {
    ...
    // @Bitmask: 0:ClearDNADatabase,1:IgnoreDNANodeConflicts,2:EnableCanfd,3:IgnoreDNANodeUnhealthy,4:SendServoAsPWM,5:SendGNSS,6:UseHimarkServo,7:HobbyWingESC,
    //             8:EnableStats,9:EnableFlexDebug,10:XiongCaiESC
    // @User: Advanced
    ...
    if (active_esc_num > 0) {
        ...
        for (uint8_t i = esc_offset; k < 8; i++) {
            if (k < 4) {
                ...
                if (armed && ((((uint32_t)1U) << i) & _ESC_armed_mask)) {
                    throtgroup2_msg_msg.command.data[k - 4] = scale_esc_output(i);
                }
                else {
        ...
        if (esc_xckj_throtgroup1.broadcast(esc_throtgroup1)) {
            _esc_send_count++;
        }
        else {
            _fail_send_count++;
        ...
        if (max_esc_num > esc_offset + 4) {
            ...
            if (esc_xckj_throtgroup2.broadcast(esc_throtgroup2)) {
                _esc_send_count++;
            }
            else {
                _fail_send_count++;
